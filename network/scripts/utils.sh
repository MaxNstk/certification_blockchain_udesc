...
function printHelp() {
  ...
  if [ "$USAGE" == "prereq" ]; then
    ...
    println "    -c <channel name> - Name of channel to create (defaults to \"certificationchannel\")"
    ...
    println "   network.sh up createChannel -bft -c certificationchannel -s couchdb "
    ...
    println "    -ccep <policy>  - (Optional) Chaincode endorsement policy using signature policy syntax. The default policy requires an endorsement from Udesc and Public"
    ...
    println "   network.sh deployCCAAS  -ccn basict -ccp ../asset-transfer-basic/chaincode -ccaasdocker false" 
  elif [ "$USAGE" == "cc" ] ; then
    println "Usage: "
